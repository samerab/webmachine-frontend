...
export const getPageById = (id: string) => createSelector(entities, (entities) => entities[id]);
export const getAllPagesSlugs = () => createSelector(allPages, (allPages) => allPages.map(page => page.info.slug));


export const currentPage = createSelector(state,(state: State) => state.currentPage);
