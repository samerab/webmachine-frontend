...
export class BlockSettingsService {
  ...
  get savedBlockSettings$(): Observable<SettingsData> {
    return this.pageSv.blockSettings$
      .pipe(
        filter(settingsData => !!settingsData && !!settingsData.settings && settingsData.sender === 'block'),
        take(1)
      )
  }

  send(settings) {
    this.pageSv.setBlockSettingsSubject.next({ sender: 'settingsForm', settings })
  }
