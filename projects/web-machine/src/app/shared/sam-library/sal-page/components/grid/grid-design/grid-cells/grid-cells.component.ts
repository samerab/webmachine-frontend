...
export class GridCellsComponent implements OnInit {
  ...
  constructor(private popupSv: PopupService, private renderer: Renderer2) {}
  ...
  extractMergeData() {
    if (this.settings?.mergeInfoList?.length) {
      ...
      for (const mi of this.settings.mergeInfoList) {
        ...
        if (
          mergeObject.minRow === mergeObject.maxRow &&
          mergeObject.minCol === mergeObject.maxCol
        ) {
  ...
  isValid(testGroup: MGroup) {
    const otherGroups = this.mergeGroupList.filter(
      (g) => g.id !== this.currentMergeGroupId
    );
  ...
  isBelongToAnotherGroup(cell: Cell) {
    const c = this.mergeGroupList
      .filter((mg) => mg.id !== this.currentMergeGroupId)
  ...
  getIntersection(anotherGroupCells: Cell[], testGroup: MGroup) {
    const anotherGroupSet = new Set([
      ...anotherGroupCells.map((c) => JSON.stringify(c)),
    ]);
    const completeGroupCells = this.getCompleteGroupCells(testGroup).map((c) =>
      JSON.stringify(c)
    );
    const intersection = new Set(
      [...completeGroupCells].filter((cell) => anotherGroupSet.has(cell))
    );
    return intersection;
  }
  ...
  groupHasCell(group: MGroup, cell: Cell) {
    const c = group?.cells.find(
      (c) => JSON.stringify(c) === JSON.stringify(cell)
    );
    if (c) return true;
    return false;
  }
  ...
  getCellIndex(group: MGroup, cell: Cell) {
    return group.cells.findIndex(
      (c) => JSON.stringify(cell) === JSON.stringify(c)
    );
  }
  ...
  isCornerCell(cell: Cell) {
    return this.getCornerCells()
      .map((c) => JSON.stringify(c))
      .includes(JSON.stringify(cell));
  }
  ...
  getCornerCells(): Cell[] {
    ...
    const cornerCellsSet = new Set(
      cornerCellsArr.map((c) => JSON.stringify(c))
    );
  ...
  getCurrentGroupIndex() {
    return this.mergeGroupList.findIndex(
      (mg) => mg.id === this.currentMergeGroupId
    );
  }
  ...
  handleGroupWithOneCell() {
    ...
    for (const index of indexes) {
      this.changeCellColor(
        this.mergeGroupList[index].cells[0],
        false,
        this.currentColor
      );
      this.mergeGroupList.splice(index, 1);
  ...
  getAllNonMergedCells() {
    ...
    const nonMergedCells = allCells
      .filter((cell) => !mergedCells.includes(cell))
