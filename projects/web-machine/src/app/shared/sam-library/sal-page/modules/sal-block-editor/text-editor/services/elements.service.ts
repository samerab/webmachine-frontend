...
export class ElementsService {
  ...
  checkMainBrs() {
    ...
    const startBrId = parts.findIndex(
      (part) => part.nodeName === 'BR' && part['id'] === 'start-br'
    );
    if (startBrId > -1) {
      if (startBrId !== 0) {
  ...
  removeMainBrs() {
    ...
    const anchorBr = this.getElementById('anchor-br');
    if (anchorBr) {
      this.renderer.removeChild(this.container, anchorBr);
    }
  }
  ...
  moveElemets(elements: HTMLElement[]) {
    for (const element of elements) {
      ...
      if (
        !elem?.parentElement ||
        elem?.parentElement?.id?.startsWith('align')
      ) {
        continue;
      }
    ...
    for (const child of childList) {
      if (child.nodeName === '#text') {
        if (child.nodeValue !== '') {
          ...
          let elem;
          if (type === 'a') {
            elem = this.createAnchorFromParent(
              child.nodeValue,
              parent,
              parentStyle
            );
  ...
  getElements(
    ...
    switch (selectionInfo.type) {
      case 'text':
        ...
        break;
      case 'div':
        const newDiv = this.createElement(
          'div',
          partList[0].nodeValue,
          parentStyle,
          newStyle
        ...
        break;
      case 'a':
        const a = this.createAnchorFromParent(
          partList[0].nodeValue,
          selectionInfo.parent,
          newStyle
        );
        elements.push(a);
        break;
  ...
  getElementInOneLine(part: ChildNode, newStyle) {
    if (part.nodeName === 'DIV') {
      ...
      return this.createAnchorFromParent(
        part.textContent,
        part as HTMLAnchorElement,
        newStyle
      );
    }
    return null;
  ...
  getElementList(part: ChildNode, newStyle): HTMLElement[] {
    ...
    if (part.nodeName === 'DIV') {
      ...
      const newElem = this.createAnchorFromParent(
        part.textContent,
        part as HTMLAnchorElement,
        newStyle
      );
  ...
  createAlignDiv(
    ...
    const alignDiv = this.createElement('div', '', null, style, preId);
    return alignDiv;
  }
  ...
  private appentToAlignDiv(
    id: any,
    lastAlignDivId: string,
    line: HTMLElement,
    alignDiv: Element
  ) {
