...
export class TextEditorComponent implements OnInit, AfterViewInit {
  ...
  private setSavedSettings() {
    this.blockSv.fetchSettingsFromBlockTemplate().subscribe((settings) => {
      if (settings?.html) {
  ...
  sendChanges() {
    this.blockSv.sendSettingsFromBlockEditor({
      html: this.content.nativeElement.innerHTML,
    });
  }

  resendChanges() {
    this.elementsSv.removeMainBrs();
    this.blockSv.sendSettingsFromBlockTemplate({
      html: this.content.nativeElement.innerHTML,
    });
  }
