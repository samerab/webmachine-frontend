...
export class GridService {
  ...
  getPopulatedSectionList(sectionList: Section[], settings: GridSettings) {
    ...
    if (sectionList.length === length) {
      ...
      return this.getSameSectionListAndAddMissing(length, sectionList, settings);
    }
    else if (sectionList.length > length) {
  ...
  private getSameSectionList(length: number, sectionList: Section[], settings: GridSettings) {
    const sections = this.generateSections(length, sectionList, settings);
    return sections;
  }

  private getSameSectionListAndAddMissing(length: number, sectionList: Section[], settings: GridSettings) {
    const sl = this.generateSections(sectionList.length, sectionList, settings);
    const missingSections = this.generateSections(length, [], settings, sectionList.length);
    const sections: Section[] = [...sl, ...missingSections];
    return sections;
  }

  private getReallocatedSectionList(length: number, sectionList: Section[], settings: GridSettings) {
    ...
    for (let index = length; index < sectionList.length; index++) {
      if (sectionList[index]?.blockList?.length && sl[length - 1]) {
        sl[length - 1].blockList = [...sl[length - 1].blockList, ...sectionList[index].blockList];
      }
    }
    return sl;
  }

  private generateSections(length: number, sectionList: Section[], settings: GridSettings, indexToStart = 0) {
    let hasCss = false;
