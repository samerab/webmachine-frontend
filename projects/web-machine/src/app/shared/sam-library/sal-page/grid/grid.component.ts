...
import { AfterViewInit, Component, OnInit, ViewChild, ViewContainerRef, Renderer2, ElementRef, ChangeDetectorRef, TemplateRef } from '@angular/core';
...
export class GridComponent implements OnInit, AfterViewInit {
  ...
  @ViewChild('sectionListContainer', { read: ViewContainerRef }) sectionListContainer: ViewContainerRef
  ...
  data: Grid;
  sectionListSubject: BehaviorSubject<Section[]> = new BehaviorSubject<Section[]>(null);
  isDesignMode;
  ...
  // private onBlockIdToDelete() {
  //   this.store.pipe(
  //     select(blockIdToDelete),
  //     filter(id => !!id)
  //   )
  //     .subscribe(id => {
  //       this.deleteBlock(id);
  //       this.store.dispatch(setBlockIdToDelete({ id: null }));
  //     });
  // }

  // deleteBlock(blockId: string) {
  //   const index = this.pageSv.getSectionIndexByBlockId(blockId, this.data.sectionList);
  //   if (index > -1) {
  //     const blockList = this.data.sectionList[index].blockList
  //     const blockIndex = blockList.map(block => block.id).indexOf(blockId);
  //     if (blockIndex > -1) {
  ...
  //     const elem = this.sectionListContainer.element.nativeElement as HTMLDivElement;
  //     show
  //       ? this.renderer.addClass(elem, 'blank-grid')
  //       : this.renderer.removeClass(elem, 'blank-grid');
  ...
  //   if (this.data.css) {
  //     const elem = this.sectionListContainer.element.nativeElement as HTMLDivElement;
  //     for (const [key, value] of Object.entries(this.data.css)) {
  //       this.renderer.setStyle(elem.parentElement, key, value);
  //     }
  //   }
  //   this.setTitleColor();
  ...
  addToFixedGrids(title) {
    this.eventSv.emit({ name: SalEventName.ADD_FIXED_GRID_TEMPLATE, value: { id: this.data.id, title } })
  }

  addToGridTemplates(title) {
    this.eventSv.emit({ name: SalEventName.ADD_GRID_TEMPLATE, value: { id: this.data.id, title } })
  }
  ...
  private toggleExpandMenuItem() {
    ...
    if (index > -1) {
      if (!this.isExpanded) {
        list[index] = { ...list[index], icon: 'expand_less', label: 'collapse' };
      }
      else {
  ...
  // updateGrid(settings) {
  //   const length = settings.cellsCount;
  //   const grid = {
  //     id: this.data.id,
  //     sectionList: this.gridSv.getPopulatedSectionList(length, this.data.sectionList),
  //     css: settings.css,
  //     otherProperties: {
  //       settings
  //     }
  ...
  deleteGrid() {
    ...
    //this.store.dispatch(setGridIdToDelete({ id: this.data.id }));
  }
