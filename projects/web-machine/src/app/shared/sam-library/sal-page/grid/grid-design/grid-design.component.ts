...
export class GridDesignComponent implements OnInit, OnDestroy {
  ...
  get rows() { return +this.form.get('rows').value }
  get cols() { return +this.form.get('columns').value }
  ...
  private onColumnsChange() {
    this.form.get('columns').valueChanges
    .pipe(filter(n => !!n)) //// TO DO: check form validation and reset columns and rows to 1
      .subscribe(n => {
        this.removeMerge();
        const columnsVal = +n;
  ...
  private onColumnsWidthChange() {
    this.form.get('columnWidthArray').valueChanges
      .subscribe(n => {
        this.setColumnsStyle();
        //this.emit();
      });
  ...
  private onRowsHeightChange() {
    this.form.get('rowHeightArray').valueChanges
      .subscribe(n => {
        this.setRowsStyle();
  ...
  setRowsStyle() {
    //if (!this.cellsDiv) return;
    const gridTemplateRows = this.form.get('rowHeightArray').value
      .map(r => {
  ...
  setRowsStyleForPreviewGrid() {
    ...
    const gridTemplateRows = this.form.get('rowHeightArray').value
      .map(r => {
        if (!r.height || r.height === '' || r.height === 'auto' || r.height < '100px') {
          return this.pageSv.defaultRowHeight;
        }
        return r.height;
      })
  ...
  setColumnsStyle() {
    if (!this.cellsDiv) return;
    const gridTemplateColumns = this.form.get('columnWidthArray').value
      .map(c => {
        if (!c.width || c.width === '') {
          return '1fr';
        }
      ...
      .join(' ');
    this.renderer.setStyle(this.cellsDiv, 'grid-template-columns', gridTemplateColumns);
    this.layout = { ...this.layout, 'grid-template-columns': gridTemplateColumns };
  }
